...
namespace OpenTPW.RSSEQ
{
    ...
    public enum OpcodeID
    {
        ...
        SETLV = 4,          // From tp.exe, number may not be correct
        ...
        ADDOBJ_EXT = 9,     // From tp.exe, number may not be correct
        ...
        EVENT_EXT = 14,     // From tp.exe, number may not be correct
        ...
        GETANIM = 20,       // From tp.exe, number may not be correct
        TRIGANIMSPEED = 21, // Unknown
        FLUSHANIM_CH = 22,  // From tp.exe, number may not be correct
        TRIGANIM_CH = 23,   // Unknown - possibly trigger animation on visitor / CHaracter
        WAITANIM_CH = 24,   // From tp.exe, number may not be correct
        LOOPANIM_CH = 25,   // Unknown - possibly loop animation on visitor / CHaracter
        TRIGWAITANIM_CH = 26,// From tp.exe, number may not be correct
        ...
        DBGMSG = 36,        // From tp.exe, number may not be correct
        ...
        PUSH = 40,          // From tp.exe, number may not be correct
        POP = 41,           // From tp.exe, number may not be correct
        HUSH = 42,          // Unknown
        HOP = 43,           // Unknown
        WAIT = 44,          // Wait for X cycles/slices/ms (unknown): WAIT <value>
        WAITABS = 45,       // From tp.exe, number may not be correct
        WAIT4ANIM = 46,     // Wait for all of the currently playing animations to finish.
        ADD = 47,           // Add a value to another: ADD <src> <value> <dest>
        MULT = 48,          // From tp.exe, number may not be correct
        DIV = 49,           // Divide values: DIV <src> <value> <dest>
        MOD = 50,           // Perform modular math on values: MOD <src> <value> <dest>
        TURBO = 51,         // From tp.exe, number may not be correct
        END = 52,           // From tp.exe, number may not be correct
        ...
        ENABLELIGHT = 82,   // From tp.exe, number may not be correct
        DISABLELIGHT = 83,  // From tp.exe, number may not be correct
        SETLIGHT = 84,      // From tp.exe, number may not be correct
        COLOURLIGHT = 85,   // From tp.exe, number may not be correct
        ...
        GETREMOTEVAR = 91,  // From tp.exe, number may not be correct
        ...
        GETCUSTPTCLCODE = 94,// From tp.exe, number may not be correct
        SETTIMER = 95,      // Unknown
        GETTIMER = 96,      // Unknown
        YEAR = 97,          // From tp.exe, number may not be correct
        MONTH = 98,         // From tp.exe, number may not be correct
        DAY = 99,           // From tp.exe, number may not be correct
